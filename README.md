...
Zed language support for the current Ember language revision.
...
1. `lsp.emberls.binary.path` (if configured)
2. `ember` from your PATH (`worktree.which("ember")`)

The extension passes `lsp` as default args. You can override args with `lsp.emberls.binary.arguments`.
...
```json
{
	"lsp": {
		"emberls": {
			"binary": {
				"path": "/home/fuad/Dev/EmberLang/Ember/bin/ember",
				"arguments": ["lsp"]
			}
		}
	}
...
Zed LSP settings are read from `emberls` via:

- `initialization_options`
- `settings`

Command launch configuration is read separately from `lsp.emberls.binary`.

The current `Ember-compiler-v2/compiler/cmd` CLI exposes an `lsp` subcommand, so no external server binary is required by default.
...
```bash
git submodule update --remote extensions/ember
```
...
## Example: Updating Ember Extension
...
# Stage and commit
git add extensions.toml extensions/ember
git commit -m "Update ember to vX.X.X"
